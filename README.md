...
明日抽籤設定在 `guildDraw/lineSchedules/tomorrowDraw`。每天設定時間只找隔天日期；沒有紀錄時將 occurrence 設為 `waiting-for-draw` 並以 `nextCheckAt` 每 5 分鐘重查，23:59 最後檢查仍不存在才標為 `expired-no-draw`，隔天使用全新的 runKey。同日期多筆仍 fail closed 為 `ambiguous-draw-records`。找到單筆 unpublished draw 後建立 canonical draw pending，並以 `📅 明日抽籤（M/D 星期X）` 明確標示結果日期與星期，再標成 `queued-for-reply`；已成功建立的 pending 不受 23:59 限制，可跨午夜等待。Reply 前會再次讀取 history：若管理員已手動 Push 或 backfill，pending 會取消而不重送。只有 LINE Reply 成功後才寫入有效 `lineSentAt`、增加 `lineSendCount`、設 `lastLineSendStatus = sent` 與 `lastLineSendMode = reply`，使既有 `isDrawPublishedToLine()` 成立；Reply 失敗則釋放 claim，抽籤仍保持 unpublished。
